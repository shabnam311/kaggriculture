...
def _count_my_animals(obs):
    """Count animals on our farm."""
    farm = _farm(obs, _seat(obs))
    count = 0
...
def _demand_this_turn(obs, item, step):
    """Calculate how many units of demand will be consumed this turn."""
    demand = 0
...
_ADAPTIVE_SELL_PRIORITY = (
    "WOOL", "MILK", "MELON", "STRAWBERRY",
    "EGG", "TOMATO", "CARROT", "FERTILIZER", "WHEAT",
)


def _adaptive_market_overlay(obs, action, step):
    action = dict(action)
    market = [list(o) for o in action.get("market", [])]
    
    # We must NEVER drop the clone's original orders!
    private = obs.get("private", {})
    shed = {k: max(0, int(v or 0)) for k, v in private.get("shed", {}).items()}
    
    for order in market:
        if len(order) >= 3 and order[0] == "SELL":
            item, qty = str(order[1]), int(order[2])
            shed[item] = max(0, shed.get(item, 0) - qty)
            
    day = step // 24
    added_sells = []
    
    # Opportunistic Fertilizer dump (Day 23+)
    if day >= 23 and shed.get("FERTILIZER", 0) > 0:
        added_sells.append(["SELL", "FERTILIZER", min(shed["FERTILIZER"], 5)])
        shed["FERTILIZER"] -= min(shed["FERTILIZER"], 5)
        
    # Terminal liquidation (Day 28+)
    if day == 29:
        for item, qty in shed.items():
            if qty > 0 and item not in ["WHEAT", "CARROT"]:
                added_sells.append(["SELL", item, min(qty, 3)])
                
    for sell in added_sells:
        if len(market) < 10:
            market.append(sell)
...
def agent(obs, configuration=None):
    ...
    try:
        ...
        # Adaptive market overlay replaces old preempt/counter/fertilizer
        action = _adaptive_market_overlay(obs, action, step)
